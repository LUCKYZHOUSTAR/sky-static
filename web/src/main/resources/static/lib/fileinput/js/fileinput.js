    ...
    FileInput.prototype = {
        ...
        resetFileStack: function () {
            ...
            self.getThumbs().each(function () {
                ...
                if (file !== undefined) {
                    ...
                    newId = self.previewInitId + '-' + i;
                    if(self.fileMap[id] != undefined) {
                        self.fileMap[newId] = self.fileMap[id];
                    }
                    $thumb.attr({
                        'id': newId,
                        'data-fileindex': i
                    });
                    delete self.fileMap[id];
                    i++;
                } else {
                    newId = 'uploaded-' + uniqId();
                    if(self.fileMap[id] != undefined) {
                        self.fileMap[newId] = self.fileMap[id];
                    }
                    $thumb.attr({
        ...
        renderFileFooter: function (caption, width) {
            ...
            if (self.isUploadable) {
                ...
                out = template.replace(/\{actions}/g, self.renderFileActions(false, true, false, false, false)) //¡¾mx¡¿updated by Sun Wang
        ...
        initPreview: function (isInit) {
            var self = this, cap = self.initialCaption || '', out, data, previewId;
            ...
            data = previewCache.data[self.id];
            if($.isArray(data.content)) {
                $.each(data.content, function(i, item) {
                    previewId = data.initId + "-init_" + i;
                    self.fileMap[previewId] = self.options.initValue[i]||{};
                })
            }
        ...
        initSuccessThumbs: function () {
            ...
            self.getThumbs('.file-preview-success').each(function () {
                ...
                handler($remove, 'click', function () {
                    var _id = $thumb.attr('id');
                    var out = self.raise('filesuccessremove', [_id, $thumb.data('fileindex'), self.fileMap[_id] || {}]);
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        ...
                        self.raise('fileremoved', [id, ind, self.fileMap[id]||{}]);
                    });
                });
            });
