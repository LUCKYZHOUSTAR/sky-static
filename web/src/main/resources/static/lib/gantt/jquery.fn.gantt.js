            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        if (entry.desc2) {
                            entries.push('<div class="row desc row' + i + ' " id="RowdId2_' + i + '" data-id="' + entry.id + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + entry.desc2 + '</span>');
                            entries.push('</div>');
                        }
                    }
                });
...
//                                    }))
            ...
            createProgressBar: function (days, cls, desc, desc2, label, dataObj) {
        ...
        var tools = {
            ...
            dateDeserialize: function (dateStr) {
                ...
                if ((/^-?\d+$/.test(settings.timezone))){
                    if(settings.timezone >= -12 && settings.timezone <= 12){
                        return new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours() + settings.timezone, date.getUTCMinutes());
                    }else{
                        return new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes());
                    }
                }else{
